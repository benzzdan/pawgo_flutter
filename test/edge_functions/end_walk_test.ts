/**
 * Edge Function integration tests: end-walk
 *
 * Run: deno test --allow-net --allow-env test/edge_functions/end_walk_test.ts
 */

const BASE_URL = Deno.env.get("SUPABASE_URL") ?? "http://localhost:8000";
const ANON_KEY = Deno.env.get("SUPABASE_ANON_KEY") ??
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0";
const FUNCTION_URL = `${BASE_URL}/functions/v1/end-walk`;
...
Deno.test("end-walk: missing booking_id → 400", async () => {
...
Deno.test("end-walk: wrong booking_id type (number) → 400 or 404", async () => {
  const token = await getAuthToken();
  if (!token) { console.warn("Skipping: no auth"); return; }
...
Deno.test("end-walk: non-existent UUID → 404", async () => {
  const token = await getAuthToken();
  if (!token) { console.warn("Skipping: no auth"); return; }
...
Deno.test("end-walk: no auth header → 401", async () => {
  const res = await fetch(FUNCTION_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json", apikey: ANON_KEY },
...
Deno.test("end-walk: invalid JWT → 401", async () => {
  const res = await fetch(FUNCTION_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
Deno.test("end-walk: GET method → 405", async () => {
  const res = await fetch(FUNCTION_URL, {
    method: "GET",
